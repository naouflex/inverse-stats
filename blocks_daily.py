...
def update_history(db_url,table_name,end_date=None):
    try:
        ...
        last_date = block_table['timestamp'].max()
        ...
        # if end_date is not provided, we set it to today
        if end_date is None:
            end_date = datetime.now().strftime("%Y-%m-%d 00:00:00")
        ...
        print(f"Update Block Table - Time elapsed: {datetime.now() - start_time}")
...
def create_current(db_url,table_name):
    ...
    try:
        ...
        for i in range(len(web3_providers)):
            ...
            if i==0:
                block_table = pd.DataFrame({'timestamp': start_time,'chain_id': chain_id, 'chain_name': chain_name, 'block_number': chain_blocks}, index=[0])
            else:
                block_table = block_table.append({'timestamp': start_time,'chain_id': chain_id, 'chain_name': chain_name, 'block_number': chain_blocks}, ignore_index=True)
        ...
        save_table(db_url,table_name,block_table)
        print(f"Create Current Block Table - Time elapsed: {datetime.now() - start_time}")
